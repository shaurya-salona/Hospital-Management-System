...
const { errorHandler, notFoundHandler, catchAsync } = require('./middlewares/errorHandler');
const { handleValidationErrors, sanitizeInput } = require('./middlewares/validation');

// Use demo database (works without PostgreSQL)
...
// Security middleware
app.use(helmet({
  contentSecurityPolicy: config.environment === 'production' ? undefined : false
}));
...
app.get('/health', (req, res) => {
  res.json({
    success: true,
    message: 'Hybrid server is healthy',
...
app.get('/api/patients', authenticateToken, (req, res) => {
  try {
    ...
    logger.error('Patients error', { error: error.message });
    res.status(500).json({
      success: false,
      message: 'Internal server error'
    });
...
app.get('/api/patients/:id', authenticateToken, (req, res) => {
  try {
    ...
    logger.error('Patient detail error', { error: error.message });
    res.status(500).json({
      success: false,
      message: 'Internal server error'
    });
...
app.get('/api/appointments', authenticateToken, (req, res) => {
  try {
    ...
    logger.error('Appointments error', { error: error.message });
    res.status(500).json({
      success: false,
      message: 'Internal server error'
    });
...
app.get('/api/analytics/patients', authenticateToken, (req, res) => {
  try {
    ...
    logger.error('Analytics patients error', { error: error.message });
    res.status(500).json({
      success: false,
      message: 'Internal server error'
    });
...
app.get('/api/analytics/financial', authenticateToken, (req, res) => {
  try {
    ...
    logger.error('Analytics financial error', { error: error.message });
    res.status(500).json({
      success: false,
      message: 'Internal server error'
    });
...
app.get('/api/analytics/operational', authenticateToken, (req, res) => {
  try {
    ...
    logger.error('Analytics operational error', { error: error.message });
    res.status(500).json({
      success: false,
      message: 'Internal server error'
    });
...
app.get('/api/patient-portal/dashboard', authenticateToken, (req, res) => {
  try {
    ...
    logger.error('Patient portal dashboard error', { error: error.message });
    res.status(500).json({
      success: false,
      message: 'Internal server error'
    });
...
app.get('/api/patient-portal/appointments', authenticateToken, (req, res) => {
  try {
    ...
    logger.error('Patient portal appointments error', { error: error.message });
    res.status(500).json({
      success: false,
      message: 'Internal server error'
    });
...
app.get('/api/patient-portal/medical-records', authenticateToken, (req, res) => {
  try {
    ...
    logger.error('Patient portal medical records error', { error: error.message });
    res.status(500).json({
      success: false,
      message: 'Internal server error'
    });
...
app.get('/api/patient-portal/prescriptions', authenticateToken, (req, res) => {
  try {
    ...
    logger.error('Patient portal prescriptions error', { error: error.message });
    res.status(500).json({
      success: false,
      message: 'Internal server error'
    });
...
app.get('/api/patient-portal/billing', authenticateToken, (req, res) => {
  try {
    ...
    logger.error('Patient portal billing error', { error: error.message });
    res.status(500).json({
      success: false,
      message: 'Internal server error'
    });
...
io.on('connection', (socket) => {
  ...
  socket.on('join-room', (data) => {
    ...
    logger.info('User joined rooms', { userId, role, socketId: socket.id });
  });
...
// Start server
server.listen(PORT, () => {
  logger.info('🚀 HMIS Hybrid Server started', {
    port: PORT,
    environment: config.environment,
    database: 'demo',
    features: {
  ...
  console.log(`\n✨ Features:`);
  console.log(`   ✅ Production logging & monitoring`);
